from utilities.assembly_utilities import local_A, experimental_local_A, find_ordering, compute_eta_x3
...
def stifness(eta_diff, sd, boundary_grid, 
             eta_dofs, quad_order, 
             chi_func: helper_chi_func.Chi_Func,
             h_dofs, conductivity):
    ...
    for c in np.arange(sd.num_cells):
        ...
        element_width = np.max(coord_loc[0, :]) - np.min(coord_loc[0, :])
        ...
        bar    = lambda a0,a1,a2, x, y: a0 + (a1 - a0) * x + (a2 - a0) * y
        h_func = lambda x,y: bar(h_local[0], h_local[1], h_local[2], x, y)
        p_func = lambda x,y: bar(ordered_coord[1, 0], ordered_coord[1, 1], ordered_coord[1, 2], x, y)
        #eta    = lambda x,y: 

        K_loc = lambda x,y: helper_chi_func.chi_quick_K_func_eval(
                        chi_func, 
                        base_height, 
                        element_height, 
                        m, 
                        eta_dofs[eta_cell], 
                        eta_dofs[eta_cell+1], 
                        grad_eta[eta_cell] / element_width, 
                        1, x, y
                    )
...
def exp_stifness(eta_diff, sd, boundary_grid, 
    ...
    for c in np.arange(sd.num_cells):
        ...
        A = experimental_local_A(coord_loc, K_loc, quad_order, m)
